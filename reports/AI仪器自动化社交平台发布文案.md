...
| 页 | 标题 | 画面内容 | 重点文字 |
|---|---|---|---|
| 1 | 一个人 + AI + 四类仪器 | 仪器实物组合，敏感信息打码 | 两套真实自动化测试闭环 |
| 2 | AI 到仪器的控制链 | 人 → AI → Python → VISA/SCPI → 仪器 | AI 负责理解和编排，程序负责执行 |
| 3 | 芯片测试链路 | DP831 → 被测件 → Keithley 2400 | 15 点、每点 10 次、共 150 次读数 |
| 4 | 安全状态机 | OFF 检查 → 限值 → 测量 → 异常关断 | 输出回读、compliance、finally 关断 |
| 5 | 信号源—示波器 | DG1022Z → MSO5354 | 正弦/方波/三角波循环切换 |
| 6 | 实时采集结果 | 波形图 + 参数表 | 47 帧 × 1000 点 = 47,000 点 |
| 7 | 开源教程 | GitHub 仓库首页或二维码 | 环境教程、Bug 台账、完整报告 |

### 理论篇：建议 6 张

| 页 | 标题 | 画面内容 | 重点文字 |
|---|---|---|---|
| 1 | AI 仪器控制的真正上限 | 从四台仪器扩展到实验室网络 | 不是 USB 口数量，而是数字化能力边界 |
| 2 | 可接入设备生态 | 电子、射频、环境、光学、机器人、工业图标 | 所有可寻址、可控制、可回读的设备 |
| 3 | 协议分层 | SCPI/VISA、OPC UA、Modbus、SiLA 2、ROS 2 | 不同协议可以组合 |
| 4 | 分布式架构 | 主控 AI + 多个边缘控制器 + 多测试站 | 主电脑不必直接连接所有设备 |
| 5 | 本地大模型 | 本地模型 + RAG + LoRA | 无云端逐 Token 费用，不等于零成本 |
| 6 | 能力与边界 | 左侧能力、右侧限制 | AI 提升编排效率，不突破物理性能和安全边界 |
...
- “已经完成四类仪器的真实联动”；
- “AI 辅助生成和修改控制逻辑”；
- “确定性 Python/VISA/SCPI 程序执行仪器动作”；
- “理论上可通过适配器和网关扩展到更多设备”；
- “本地推理不再产生云端逐 Token 费用”。
